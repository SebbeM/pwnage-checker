...
pub fn run(params: Params) -> Result<(), Box<dyn Error>> {
    ...
    let pass: [u8; 40] = hashed_pass.as_bytes().try_into().unwrap();

    let mut file: File = File::open(params.path.clone()).unwrap();

    let len = file.metadata().unwrap().len();
    println!("The file is {} bytes long", len);

    match binary_search(file, pass, len) {
        Some(index) => println!("Binary search found at index: {}", index),
        None => println!("Password \"{}\" not found", "password"),
    }
    ...
    while low <= high {
        ...
        match res {
            ...
            Ok(_) => match token.cmp(buf) {
                Ordering::Equal => return Some(usize::try_from(mid).unwrap()),
                Ordering::Less => {
